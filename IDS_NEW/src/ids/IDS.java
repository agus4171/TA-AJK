                    ...
                    if (datasetTcp.size() != 0 | datasetUdp.size() != 0) {
                        ...
                        for (DataPacket dataPacketTes : dataTest) {     
                            ...
                            if ("tcp".equals(dataPacketTes.getProto())) {                                
                                ...
                                for (DataPacket dataSetA : datasetTcp) {
                                    if (dataSetA.getSrcIP().equals(dataPacketTes.getSrcIP()) && dataSetA.getDstPort() == dataPacketTes.getDstPort()) {
                                        sd = new Mahalanobis(dataSetA.getNgram());                                        
                                        dist = sd.distance(dataPacketTes.getNgram(), dataSetA.getNgram(), sFactor);
                                        if (dist > threshold) {
                                            System.out.println("tcp "+dataSetA.getType()+" "+dataPacketTes.getType());
//                                            if (dataSetA.getType() > 0) {
//                                                System.out.println("tcp "+dataSetA.getType());
//                                            }                                            
                                        }
                                        val.add(dist);
                                        if (dataPacketTes.getDstPort() == 20) {
                                            file = new File(System.getProperty("user.dir")+"/threshold/DstPort_20");
                                            if (!file.exists()) {
                                                file.createNewFile();
                                            }
                                            fw = new FileWriter(file, true);
                                            fw.append("tcp -> "+dataSetA.getSrcIP()+"-"+dataSetA.getSrcPort()+"-"+dataSetA.getDstIP()+"-"+dataSetA.getDstPort()+" -> "+dist+"\n");
                                            fw.close();
                                            
                                        } 
                                        
                                        else if (dataPacketTes.getDstPort() == 21) {
                                            file = new File(System.getProperty("user.dir")+"/threshold/DstPort_21");
                                            if (!file.exists()) {
                                                file.createNewFile();
                                            }
                                            fw = new FileWriter(file, true);
                                        ...
                                        else if (dataPacketTes.getDstPort() == 22) {
                                            file = new File(System.getProperty("user.dir")+"/threshold/DstPort_22");
                                            if (!file.exists()) {
                                                file.createNewFile();
                                            }
                                            fw = new FileWriter(file, true);
                                        ...
                                        else if (dataPacketTes.getDstPort() == 25) {
                                            file = new File(System.getProperty("user.dir")+"/threshold/DstPort_25");
                                            if (!file.exists()) {
                                                file.createNewFile();
                                            }
                                            fw = new FileWriter(file, true);
                                        ...
                                        else if (dataPacketTes.getDstPort() == 80) {
                                            file = new File(System.getProperty("user.dir")+"/threshold/DstPort_80");
                                            if (!file.exists()) {
                                                file.createNewFile();
                                            }
                                            fw = new FileWriter(file, true);
                                            ...
                                            file = new File(System.getProperty("user.dir")+"/threshold/DstPort_other");
                                            if (!file.exists()) {
                                                file.createNewFile();
                                            }
                                            fw = new FileWriter(file, true);
                                ...
                                if (!val.isEmpty()) {
                                    maxt = val.stream().max(Double::compare).get();
                                    mint = val.stream().min(Double::compare).get();
                                    fwFree.append(val.size()+" "+maxt+" & "+mint);
                                    fwFree.append("\n");
                                }                                                                
                            }
                                ...
                                for (DataPacket dataSetB : datasetUdp) {   
                                    if (dataSetB.getSrcIP().equals(dataPacketTes.getSrcIP()) && (dataSetB.getDstPort() == dataPacketTes.getDstPort())) {
                                        sd = new Mahalanobis(dataSetB.getNgram());
                                        dist = sd.distance(dataPacketTes.getNgram(), dataSetB.getNgram(), sFactor);
                                        if (dist > threshold) {
                                            System.out.println("tcp "+dataSetB.getType()+" "+dataPacketTes.getType());
//                                            if (dataSetB.getType() > 0) {
//                                                System.out.println("udp "+dataSetB.getType());
//                                            }  
                                        }
                                        val.add(dist);
                                        file = new File(System.getProperty("user.dir")+"/threshold/DstPort_udp");
                                        if (!file.exists()) {
                                            file.createNewFile();
                                        }
                                        fw = new FileWriter(file, true);
                                        fw.append("udp -> "+dataSetB.getSrcIP()+"-"+dataSetB.getSrcPort()+"-"+dataSetB.getDstIP()+"-"+dataSetB.getDstPort()+" -> "+dist+"\n");
                                        fw.close();
                                    }                                     
                                }
                                ...
                                if (!val.isEmpty()) {
                                    maxu = val.stream().max(Double::compare).get();
                                    minu = val.stream().min(Double::compare).get();
                                    fwFree.append(val.size()+" "+maxu+" & "+minu);
                                    fwFree.append("\n");
                                }                                                              
                            }
                        }
